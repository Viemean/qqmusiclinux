...
└── 4. 体验打磨与细节改进 (Enhancements & Improvements)
    ├── [x] [Enh 1] Linux 原生桌面切歌通知 (D-Bus 带封面气泡弹窗，必须可配置/可快捷键随时关闭)
    ├── ~~[Enh 2] 歌词时间轴实时微调与偏好持久化 (需求取消)~~
    └── [x] [Enh 3] 离线音频标签就地注入与自包含无损歌曲导出 (ATL.NET 读写闭环)
```
...
[Milestone 4: 系统桌面级体验打磨] (细节雕琢)
   Step 4.1: [x] [Enh-01] Linux 原生切歌通知 (D-Bus 弹窗，严格支持配置/快捷键关闭)
   Step 4.2: ~~[Enh-02] 歌词微调与偏好持久化 (需求取消)~~
   Step 4.3: [x] [Enh-03] 离线音频标签就地注入与自包含导出
```
...
- [x] **[Enh-01] Linux 原生桌面切歌通知 (D-Bus Notifications，严格支持可关闭)**
  - [x] 纯 C# 原生调用 D-Bus `org.freedesktop.Notifications` 接口；
  - [x] 切歌时弹出气泡，展示歌名、歌手、音质徽标及已缓存的封面缩略图；
  - [x] **必须具备完全可关闭性**：
    - [x] 在用户配置文件（`~/.config/qqmusic-tui/config.json`）中提供 `"enable_song_switch_notification": true/false`；
    - [x] 支持 CLI 启动参数 `--no-notify` 快速静音覆盖；
    - [x] 在快捷键或菜单中提供全局一键开/关切歌通知，避免在免打扰、全屏编码或打游戏时弹窗干扰。
- [x] ~~**[Enh-02] 歌词时间轴实时微调与偏好持久化 (需求取消)**~~
  - ~~在大播放界面支持 `[`（歌词提前 0.5s）与 `]`（歌词延后 0.5s）实时调整；~~
  - ~~将微调的 offset 值持久化到本地歌曲配置，下次播放自动对齐。~~
- [x] **[Enh-03] 离线音频标签就地回写与自包含无损歌曲导出**
  - [x] 命中本地无损缓存后执行零开销文件复制；
  - [x] 依托 `ATL.NET` 的原生写入能力（`track.Save()`），将云端拉取的高清封面、双语歌词与元数据就地写入文件头，导出全自包含的独立无损音频。
