...
- [ ] **[Fix-01] 核心播放队列与浏览视图解耦治理 (P0 核心修复)**
  - [ ] 问题根因：当前切歌强依赖 `_songListView.Songs`，用户浏览搜索结果或歌手页时，原歌单播放完毕会被错误篡改为新浏览页的歌曲。
  - [ ] 治理方案：建立独立的 `PlaybackQueueService`，将活跃待播队列与临时渲染视图物理隔离，浏览任何页面均不破坏正在播放的歌单流。
- [ ] **[Fix-02] 切歌瞬间数字爆音治理**
  - [ ] 治理方案：在 GStreamer 管道中引入 `volume` 线性渐变控制，切歌瞬间在 150ms 内执行极速软淡出，切入新流后 150ms 软淡入，消除硬切引起的数字破音。
- [ ] **[Fix-03] 弱网断线与笔记本合盖唤醒恢复缺陷治理**
  - [ ] 治理方案：监听 GStreamer Buffer 枯竭与 Socket 断连事件，建立自动重连看门狗（Watchdog），支持最多 3 次带指数退避（1s, 2s, 4s）的微秒断点自动续播。
...
- [ ] **[Feat-02] 主列表内即时查找与行去重跳转 (G 键浮窗)**
  - [ ] 全局单键绑定：在 `MainWindow.cs` 全局按键中捕获 `c == 'G'` 唤出/收起浮动查找窗。
  - [ ] 悬浮居中面板装配：宽 50 列、高 3 行，翡翠绿边框，内置 `TextField` 与匹配计数 `[ X / Y ]`。
  - [ ] 行粒度去重算法：以歌曲条目行索引（`Row Index`）为唯一录入单位，遍历比对 `Title || Artist || Album`，单行内多字段命中绝不重复计次。
  - [ ] 连续 Enter 环形跳转状态机：首次 Enter 定位第一项，后续 Enter 顺序跳转并在末尾折返，`Shift+Enter`/`Up` 反向跳跃，`Esc` 退出保留光标位置。
  - [ ] 行内切片高亮渲染。
...
- [ ] **[Feat-04] 插队播放 (Play Next) 与播放队列抽屉 (Queue Drawer)**
  - [ ] 在列表条目上按快捷键 `N`，将选定歌曲直接插队至当前播放队列的下一顺位。
  - [ ] 呼出独立播放队列抽屉，支持查看后续待播歌曲清单与移除指定条目。
